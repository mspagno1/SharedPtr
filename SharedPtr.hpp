...
namespace cs540{
	...
	public:
		...
		template <typename U>SharedPtr<T> &operator=(const SharedPtr<U> & shrPtr){
			//If we are not self assigning 
			
			if(this != ((SharedPtr<T>*)&shrPtr)){
				pthread_mutex_lock(&counter_mutex);
				//Clearing this pointers connection to the old data if it was the last pointer
				if(this->cBlock != NULL){
					//If it was the last pointer we clear the controlBlock
	...
	bool operator==(const SharedPtr<T1> &shrPtr1, const SharedPtr<T2> &shrPtr2){
	...
	template <typename T>
	bool operator==(const SharedPtr<T> & shrPtr, std::nullptr_t nullP){
	...
	template <typename T>
	bool operator==(std::nullptr_t nullP, const SharedPtr<T> & shrPtr){	
		if(shrPtr.data == nullP){
			return true;
		}
		else{
